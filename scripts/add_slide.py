...
Duplicates an existing slide or creates a blank one from a slide layout.
Handles rId management, Content_Types.xml, and presentation.xml.rels
automatically — never copy slide XML files manually.

Ported verbatim from the Anthropic PPTX skill (add_slide.py).

Usage:
    python scripts/add_slide.py <unpacked_dir> <source>

The source can be:
  - A slide file  (e.g., slide2.xml)       — duplicates the slide
  - A layout file (e.g., slideLayout2.xml) — creates blank from layout
...
# ---------------------------------------------------------------------------
# Slide creation strategies
# ---------------------------------------------------------------------------

def create_slide_from_layout(unpacked_dir: Path, layout_file: str) -> None:
    """Create a blank slide wired to an existing slide layout."""
...
def duplicate_slide(unpacked_dir: Path, source: str) -> None:
    """Duplicate an existing slide — manages rIds and rels correctly."""
    slides_dir = unpacked_dir / "ppt" / "slides"
    rels_dir = slides_dir / "_rels"
    ...
    if source_rels.exists():
        ...
        # Strip notes-slide relationships from the copy — each slide needs its own
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python scripts/add_slide.py <unpacked_dir> <source>", file=sys.stderr)
