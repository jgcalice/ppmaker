...
Ported verbatim from the Anthropic PPTX skill (clean.py).

This script removes:
- Orphaned slides (not in sldIdLst) and their .rels
- [trash] directory
...
# ---------------------------------------------------------------------------
# Slide inventory helpers
# ---------------------------------------------------------------------------

def get_slides_in_sldidlst(unpacked_dir: Path) -> set[str]:
    """Return the set of slide filenames that are registered in sldIdLst."""
...
def remove_orphaned_slides(unpacked_dir: Path) -> list[str]:
    ...
    if removed and pres_rels_path.exists():
        ...
        if changed:
            with open(pres_rels_path, "wb") as fh:
                fh.write(rels_dom.toxml(encoding="utf-8"))
